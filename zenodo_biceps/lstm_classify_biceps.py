"""
LSTM sequence classifier: biceps fatigue STATE, built on classify_biceps.py.
=============================================================================

Adds a deep-learning option alongside the existing RF/SVM/KNN classifiers, per
the supervisor's "this break: LSTM pattern classification and prediction" plan
(next semester: transformer).

Reuses the SAME data pipeline as classify_biceps.py so results are directly
...
becomes one timestep. An LSTM consumes a CAUSAL sequence of the last
`--seq-len` windows (current window + history, default 6 -- the same span as
classify_biceps's --temporal-history 5) and either:
  - classifies the CURRENT window's fatigue state (default). This is the same
    task RF/SVM/KNN solve with hand-engineered delta/slope features -- here
    the LSTM learns the temporal pattern itself from the raw feature sequence.
  - predicts the NEXT window's fatigue state (--predict-next), the
    "prediction" half of the supervisor's brief.

Optional --finetune adds a transfer-learning step: after LOSO pretraining,
fine-tune a copy of the model on an early calibration slice of the held-out
subject's own (time-ordered) sequences, then evaluate on the remainder. This
is reported SEPARATELY from the main LOSO number -- the two are never mixed,
so the headline accuracy stays an honest subject-held-out figure.

Usage:
    python lstm_classify_biceps.py --root sEMG_data --side R
    python lstm_classify_biceps.py --root sEMG_data --side R --label-mode binary_drop_transition
    python lstm_classify_biceps.py --root sEMG_data --side R --predict-next
    python lstm_classify_biceps.py --root sEMG_data --side R --finetune
    python lstm_classify_biceps.py --root sEMG_data --side R --json-out out/metrics_lstm_3class_250hz_m4.json
"""
...
# ---------------------------------------------------------------------------
# per-window features -> causal sequences
# ---------------------------------------------------------------------------
...
class LSTMClassifier(nn.Module):
    def __init__(self, n_features: int, hidden_size: int = 64,
                 num_layers: int = 1, n_classes: int = 3, dropout: float = 0.0):
        super().__init__()
...
def train_model(model: LSTMClassifier, X: np.ndarray, y: np.ndarray,
               n_classes: int, epochs: int, lr: float, device,
               batch_size: int = 64) -> LSTMClassifier:
...
@torch.no_grad()
def predict(model: LSTMClassifier, X: np.ndarray, device) -> np.ndarray:
    model.eval()
    X_t = torch.tensor(X, dtype=torch.float32, device=device)
...
# ---------------------------------------------------------------------------
def run_loso_lstm(subs, Seq_by, y_by, n_classes, hidden_size, num_layers,
                  dropout, epochs, lr, batch_size, device, seed) -> dict:
    ...
    for test_s in subs:
        ...
        model = LSTMClassifier(n_features=Xtr.shape[-1], hidden_size=hidden_size,
...
def run_loso_lstm_finetune(subs, Seq_by, y_by, n_classes, hidden_size, num_layers,
                           dropout, epochs, lr, batch_size, device, seed,
    ...
    for test_s in subs:
        ...
        torch.manual_seed(seed)
        base = LSTMClassifier(n_features=Xtr.shape[-1], hidden_size=hidden_size,
                              num_layers=num_layers, n_classes=n_classes,
                              dropout=dropout).to(device)
...
def main():
    ...
    ap.add_argument("--seq-len", type=int, default=6,
                    help="causal window history length fed to the LSTM "
    ...
    ap.add_argument("--json-out", default=None,
                    help="optional metrics JSON path (same schema as "
                         "classify_biceps.py so make_classifier_report.py "
                         "picks it up)")
    args = ap.parse_args()
    ...
    for s in range(1, 14):
        data = cb.subject_windows(
            ...
            temporal=False,   # raw per-window features; the LSTM learns the
                              # temporal pattern itself from the sequence
    ...
    print(f"\n=== LSTM LOSO ({args.label_mode}, {fs_label}Hz, "
          f"seq-len={args.seq_len}, {task_label}) ===")
    print(f"features per step: {n_features}")

    res = run_loso_lstm(subs, Seq_by, y_by, n_classes, args.hidden, args.layers,
                        args.dropout, args.epochs, args.lr, args.batch_size,
                        device, args.seed)
    ...
    if args.finetune:
        ...
        zs, ft, used = run_loso_lstm_finetune(
            subs, Seq_by, y_by, n_classes, args.hidden, args.layers,
            args.dropout, args.epochs, args.lr, args.batch_size, device,
            args.seed, args.calib_frac, args.ft_epochs, args.ft_lr)
        ...
        results["LSTM_transfer_zeroshot"] = zs
        results["LSTM_transfer_finetuned"] = ft

    print(f"\n{'Model':<24} {'Mean Acc':>10} {'Std Acc':>10} {'Mean F1':>10}")
    print("-" * 56)
    for name, r in results.items():
        print(f"{name:<24} {r['mean_acc']:>10.3f} {r['std_acc']:>10.3f} {r['mean_f1']:>10.3f}")

    if args.json_out:
        temporal_label = f"LSTM(seq={args.seq_len})"
