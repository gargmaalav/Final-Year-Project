...
_CONVERSATION_KEYS = {
    ...
    "awaiting_subject": False,   # whether "5" on its own names a subject
}
...
def _name_the_subject(user_text: str) -> str:
    """"5" -> "subject 5", but only when a subject was just asked for."""
    if not st.session_state.awaiting_subject:
        return user_text
...
def _handle_turn(user_text: str, uploaded_file) -> None:
    ...
    # what the reader typed is shown; what the pipeline sees is the resolved
    # form, so the transcript still reads as the conversation they had
    user_text = _name_the_subject(user_text)
    ...
    st.session_state.awaiting_subject = bool(
